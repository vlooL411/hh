...
const SessionProvider = ({ children,
    callbackSignIn = '/', callbackSignOut = '/signin' }: Props) => {
    const [user, setUser] = useState(null)
    const router = useRouter()

    useEffect(() => {
        const userLocalStorage = JSON.parse(localStorage.getItem(userItem)) as UserSafe
    ...
    return <SessionContext.Provider value={{ user, setUser: SetUser, remUser }}>
        {children}

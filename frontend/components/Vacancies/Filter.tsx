...
const Filter = ({ onChange = () => { } }: Props): ReactElement => {
    ...
    return <div className={filter}>
        <input ref={jobRef} onChange={OnChange}
            placeholder='Job title' defaultValue={query?.name} />
        <input ref={cityRef} onChange={OnChange}
            placeholder='City' defaultValue={query?.city} />
        <input ref={salaryRef} onChange={OnChange} type='number'
            placeholder='Salary' defaultValue={query?.from} />
    </div>
}
